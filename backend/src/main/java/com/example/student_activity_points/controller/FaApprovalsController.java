...
public ResponseEntity<?> getFaDetails(@RequestParam String email) {
    ...
    Integer faId = fa.getFAID();

    // Step 1: Get all requests assigned directly to this FA (where approval is still pending)
    List<Long> assignedRequestIds = requestedRepository.findByFAID(faId).stream()
    ...
    List<Student> myStudents = studentRepository.findByFAID(faId);
